...
class ElfFile:
    """
    A Class that represents a file in an Elf Directory
    """
    ...
    def parse(input: str):
        tokens = input.split()
        return ElfFile(tokens[1], int(tokens[0]))

    @staticmethod
    def is_parseable(input: str):
        tokens = input.split()
        return len(tokens) == 2 and str.isdecimal(tokens[0])


class ElfDirectory:
    """
    A Class that represents an Elf Directory
    """
    ...
    @staticmethod
    def parse(input: str, parent: 'ElfDirectory'):
        tokens = input.split()
        return ElfDirectory(name=tokens[1], parent=parent)

    @staticmethod
    def is_parseable(input):
        tokens = input.split()
        return len(tokens) == 2 and tokens[0] == "dir"
...
def process_directory_line(current_dir: ElfDirectory, input: str):
    ElfDirectory.parse(input, current_dir)


def process_file_line(current_dir: ElfDirectory, input: str):
    file = ElfFile.parse(input)
    current_dir.add_file(file)


def process_change_directory(current_dir: ElfDirectory, input: str):
    child_name = input.split()[2]
    if child_name == "..":
        return current_dir.parent
    elif child_name == "/":
        return current_dir

    kids = current_dir.children
    for c in kids:
...
def build_filesystem(input):
    root = ElfDirectory(name="/", parent=None)
    current_dir = root

    for line in input:
        if ElfFile.is_parseable(line):
            process_file_line(current_dir, line)
        elif ElfDirectory.is_parseable(line):
            process_directory_line(current_dir, line)
        elif line.startswith("$ cd"):
            current_dir = process_change_directory(current_dir, line)

    return root


def find_all_directories(directory: ElfDirectory):
    all_dirs = [directory]

    children = directory.get_children()

    for i in children:
        for j in find_all_directories(i):
            all_dirs.append(j)
    return all_dirs
...
def do_part_one():
    input = get_input()
    file_system = build_filesystem(input)
    dirs = find_all_directories(file_system)
...
def do_part_two():
    ...
    in_use = dirs[0].size()
    free_space = file_system_total - in_use
    # 18381165
    print(f"free space on device: {free_space}")

    if free_space < space_needed:
        # search
        print("need to clean up")
        need_to_clean = space_needed - free_space
        print(f"need to remove this much space: {need_to_clean}")

        the_list = {}
        for dir in dirs:
            size = dir.size()
            if size >= need_to_clean:
                the_list[dir.name] = size

        print(dict(sorted(the_list.items(), key=lambda item: item[1])))
