...
// readFileAsString Read a file and return the contents as a string
...
// ParseMemoryForMulInstructions Parse the memory for mul instructions
func ParseMemoryForMulInstructions(input string) [][]string {
	// Parse the input string and extract all the valid mul instructions
...
func ParseOperandsFromMulInstructions(instructions [][]string) [][]int {
	...
	for _, instruction := range instructions {
		...
		for i := 1; i < len(instruction); i++ {
			num, err := strconv.Atoi(instruction[i])
			if err != nil {
...
// CalculateMultiplication Calculate the multiplication of the operands
func CalculateMultiplication(operands [][]int) int {
	var result int
