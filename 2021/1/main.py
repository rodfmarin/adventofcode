...
def do_part_one():
    ...
    # iterate over every entry and check if this entry is increasing over the last entry
...
def do_part_two():
    # This got a bit trickier, in general it is still counting when values are incrementing
    # We now need to group values into a sliding window of 3 and compare with the previous window then count if
    # increasing

    # the input isn't guaranteed to be divisible by 3, the instructions say stop when there is no more values left
    # to create a 3 value window
    ...
    # for A we step 3
    # for B we step A starting value + 1 for 3 positions
    # for C we step B starting value + 1 for 3 positions
    # for X we step Y starting value + 1 for 3 positions
    # I think if we blindly walk the list then we'll hit an out of bounds error, we could try to catch that and signal
    # end of list.
    ...
    try:
        for i in range(0, len(input)):
            ...
            window_a = sum(input[i:i+3])
            # create a second window
            window_b = sum(input[i+1:i+4])
            # compare them
